...
const Contact = () => {
  ...
  const getColorClasses = (color: string) => {
    const colors = {
      ...
      green:
        "text-green-400 bg-green-600/20 hover:bg-green-600/30 border-green-500/30",
          ...
          {contactMethods.map((method, index) => (
            ...
            >
              <div className="flex items-start space-x-4">
                <div
                  className={`p-3 rounded-xl ${getColorClasses(method.color)} group-hover:scale-110 transition-transform duration-300`}
                >
                  {method.icon}
                </div>
